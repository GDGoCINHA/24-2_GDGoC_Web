      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {sections.map((section) => (
            ...
            >
              <h2 className="text-2xl font-semibold mb-3 text-slate-900 group-hover:text-blue transition">
                {section.title}
              </h2>
              <p className="text-slate-600 text-sm mb-6">{section.description}</p>
              <span className="inline-flex items-center gap-2 text-sm text-blue group-hover:gap-3">
                바로가기 →
              </span>
            </Link>
          ))}
